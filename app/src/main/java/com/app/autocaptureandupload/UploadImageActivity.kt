...
class UploadImageActivity : AppCompatActivity() {
    ...
    private var timeerVal: Int = 0
    private var captureVar: Int = 0
    ...
    private val onImageAvailableListener = ImageReader.OnImageAvailableListener { reader ->
        ...
        if (pictureFile != null) {
            ...
            fos.close()
            image?.close()
            println("Calture Value:- $captureVar")
...
//
                timeerVal += 1
                println("Timer Value:- $timeerVal")
                val rotation = windowManager.defaultDisplay.rotation
                capReq.set(CaptureRequest.JPEG_ORIENTATION, orientations.get(rotation))
                captureSession.capture(capReq.build(), captureCallbackHandler,
                    handler)
